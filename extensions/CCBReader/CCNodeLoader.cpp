...
// #endif
    ...
    {
        this->onHandlePropTypeSize(pNode, pParent, PROPERTY_PREFEREDSIZE, preferedSize, pCCBReader);
    }
}
...
CCNode * CCNodeLoader::parsePropTypeCCBFile(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
    ...
    //If there is no data, search in the ccbi/ folder (custom addition)
    if(size == 0)
    {
        ccbFileName = "ccbi/" + ccbFileName;
        path = CCFileUtils::sharedFileUtils()->fullPathForFilename(ccbFileName.c_str());
