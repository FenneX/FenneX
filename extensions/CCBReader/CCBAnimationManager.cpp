...
void CCBAnimationManager::runAnimationsForSequenceIdTweenDuration(int nSeqId, float fTweenDuration)
{
    ...
    {
        CCNode *node = (CCNode*)pElement->getIntKey();
        //Commented because it's not practical if you want to run another animation alongside the CCB animation
        //node->stopAllActions();
...
{
    CCAssert(0, "CCBSoundEffect::update not implemented because of linking problem with CocosDenshion");
